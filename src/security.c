...
\"'";
    ...
    for (size_t i = 0; i < sizeof(dangerous_names) / sizeof(dangerous_names[0]); i++) {
        if (strcasecmp(filename, dangerous_names[i]) == 0) {
            return 0;
        }
    }
...
int security_create_checksum_file(const char *file_path, const char *checksum_path) {
    ...
    char hash[256];
    int result = security_calculate_checksum(file_path, hash, sizeof(hash), HASH_TYPE_SHA256);
    
    if (result != TINYPKG_SUCCESS) {
        return result;
    }
    
    FILE *fp = fopen(checksum_path, "w");
    if (!fp) {
        log_error("Failed to create checksum file: %s", checksum_path);
        return TINYPKG_ERROR_FILE;
    }
    
    char *basename = utils_get_basename(file_path);
    fprintf(fp, "%s  %s\n", hash, basename ? basename : file_path);
    fclose(fp);
    
    if (basename) TINYPKG_FREE(basename);
    
    log_debug("Created checksum file: %s", checksum_path);
    return TINYPKG_SUCCESS;
}
